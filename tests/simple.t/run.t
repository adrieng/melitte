  ...
  eval (suc c 0 : ℕ)
  val add : ℕ → ℕ → ℕ =
    λ n m ⇒ elim ℕ n with _ ⇒ ℕ { zero ⇒ m | suc _, r ⇒ suc r }
  eval add 2 5
  val mul : ℕ → ℕ → ℕ =
    λ n m ⇒ elim ℕ n with _ ⇒ ℕ { zero ⇒ 0 | suc _, r ⇒ add m r }
  eval mul 3 10
  {- Elaborated code -}
  val c : ∀ (_0 : ℕ) → ℕ = λ n ⇒ n
  eval (1 : ℕ)
  val add : ∀ (_1 : ℕ) (_2 : ℕ) → ℕ =
    λ n m ⇒ elim ℕ n with _3 ⇒ ℕ { zero ⇒ m | suc _3, r ⇒ suc r }
  eval (7 : ℕ)
  val mul : ∀ (_2 : ℕ) (_3 : ℕ) → ℕ =
    λ n m ⇒ elim ℕ n with _4 ⇒ ℕ { zero ⇒ 0 | suc _4, r ⇒ add m r }
  eval (30 : ℕ)
  ...
  val iter : ∀ (A : 𝕌 0) (f : ∀ (_5 : A) → A) (_6 : A) (_7 : ℕ) → A =
    λ A f z n ⇒ elim ℕ n with _8 ⇒ A { zero ⇒ z | suc _8, r ⇒ f r }
...
Melitte contains finite types.
  ...
  $ melitte t6.melitte
  $ melitte t7.melitte
  File "t7.melitte", line 1, characters 19-20:
  this expression has type 1 but a type of shape Nat or Fin (Suc _) was expected
  [1]
